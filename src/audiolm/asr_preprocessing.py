...
parser.add_argument("--audio_processor", type=str, default="kyutai/mimi",
...
parser.add_argument("--sampling_rate", type=int, default=24000, help="Target sampling rate for audio.")
...
def prepare_dataset(batch, model, audio_processor, text_tokenizer):
    ...
    codes = outputs.audio_codes[0,:,:]
    batch["input_features"] = codes.cpu().numpy()
...
def make_asr_dataset(args: argparse.Namespace) -> None:
    audio_processor = AutoFeatureExtractor.from_pretrained(args.audio_processor)
    text_tokenizer = AutoTokenizer.from_pretrained(args.text_tokenizer)
    model = MimiModel.from_pretrained(args.audio_processor)
    model.eval()
